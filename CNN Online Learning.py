# ============================================================
# ExNAS para CNN (SmallCNN) - Colab Ready (CPU) + Calibración + Fallback
# ============================================================
...
class LayerMemory:
    """
    Firmas 1D (CPU) con top-k por similitud coseno y decay temporal.
    Si 'dim' es None, usa la dimensión original.
    """
    def __init__(self, dim=None, top_k=16, decay=0.95, maxlen=1024):
        self.dim = dim
        self.top_k = top_k
        self.decay = decay
        self.signatures = deque(maxlen=maxlen)  # tensores 1D en CPU
        self.scores = deque(maxlen=maxlen)      # floats
        self.projector = None                   # (F x dim) para proyección opcional
    ...
    def _compress(self, features):
        if features.ndim == 4:  # Conv: [B,C,H,W] → (C,)
            ...
            return sig
        # FC: [B,F] → (F,), con proyección opcional
...
# ---------- Calibración de la puerta Conv2 (Ridge) ----------
@torch.no_grad()
def calibrate_gate_conv2_linear(base_model, loader, steps=8, lam=1e-3, device="cpu"):
    ...
    for xb, _ in loader:
        ...
        Y = x2.pow(2).mean(dim=(2, 3)).sqrt()                   # (B,64) L2 espacial
    ...
    return W.T.contiguous()                                     # devolver como (64,32) (peso de Linear)

# ---------- ExNAS compute-aware + fallback ----------
class ExNASWrapper(nn.Module):
    """
    Gating compute-aware:
      - Conv2: subset de out-ch antes de computar la capa.
      - FC:   subset de features (columnas de W).
    Extras:
      - Memoria estable (basada SIEMPRE en Conv1).
      - Calibración previa de gate_conv2 (opcional).
      - Fallback por incertidumbre: reevalúa sólo las muestras dudosas con baseline.
    """
    def __init__(self, base_model,
                 ...
                 fallback_thresh=0.60   # reevalúa con baseline si max prob < 0.60
                 ):
        super().__init__()
        ...
        # Memorias
        self.mem_conv2 = LayerMemory(dim=None,       top_k=top_k, decay=mem_decay)  # usa Conv1 (32)
        self.mem_fc    = LayerMemory(dim=mem_dim_fc, top_k=top_k, decay=mem_decay)

        # Puerta barata para Conv2: GAP Conv1 → Linear(32→64)
    ...
    def forward(self, x0):
        ...
        # actualizar memoria de Conv2 SIEMPRE con Conv1
        with torch.no_grad():
        ...
        # ---- Fallback por incertidumbre (opcional) ----
...
def run_experiment(
    ...
    train_epochs=1,
    batch_size=128,
    # Presupuestos más conservadores para mejor Acc/Speed
    bl=0.20, Bg=0.05,
    top_k=16,
    mem_dim_fc=128,
    mem_decay=0.95,
    ...
    # Calibración rápida de la puerta Conv2 (mejora selección de canales)
...
if __name__ == "__main__":
    run_experiment(
        ...
        train_epochs=1,
        batch_size=128,
        bl=0.20, Bg=0.05,        # buen punto de partida
        top_k=16,
        mem_dim_fc=128,
        mem_decay=0.95,
        min_keep_conv2=24,       # ↑ si quieres más accuracy
        min_keep_fc=8192,        # ↑ si quieres más accuracy
        fallback_thresh=0.60,    # baja a 0.55 si quieres más speed
        calib_steps=8, calib_lam=1e-3,
        num_workers=2
    )
